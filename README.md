...
Apache-2.0 rather than MIT for two reasons specific to this project: it carries
an express patent grant, which matters when implementing someone else's protocol;
and it makes preserving `NOTICE` a condition of redistribution, so the record of
what was and was not derived from GPL-licensed kernel drivers travels with the
code.
